...
export default function ViewBorrowForm() {
    ...
    useEffect(() => {
        const fetchTransaction = async () => {
            try {
                if (type !== 'borrow' && type !== 'reservation') {
                    throw new Error("Invalid form type");
                }
                const response = await api.get(`/public/${type}/${id}`);
                const data = response.data;
                // Normalize returnDate to dueDate for reservation format
                if (type === 'reservation' && data.returnDate) {
                    data.dueDate = data.returnDate;
                }
                setTransaction(data);
            } catch (error) {
                console.error("Error fetching transaction", error);
                alert("ไม่พบข้อมูลเอกสาร หรือลิงก์อาจหมดอายุ");
                navigate('/');
            } finally {
                setLoading(false);
            }
    ...
    return (
        <div className="bg-gray-200 text-black font-sans min-h-screen pb-10">
            ...
            <div className="no-print p-4 bg-gray-100 flex justify-between items-center shadow-md mb-8 sticky top-0 z-50">
                <button onClick={() => navigate('/')} className="px-4 py-2 bg-gray-500 text-white rounded hover:bg-gray-600 font-bold">
                    &larr; กลับหน้าหลัก
                </button>
                <div className="text-sm text-gray-600 bg-yellow-100 p-2 rounded text-center mx-4">
                    เอกสารฉบับนี้สำหรับพิมพ์หรือบันทึกเป็น PDF โปรดตั้งค่า <b>Paper size: A4</b>, <b>Margins: None</b> และ <b>Background graphics</b>
                </div>
